...
// `taskkill` ignores non-zero signals (`/F` terminates the tree).
const killDescendantsWindows = (subprocess, signal) => {
	if (subprocess.pid === undefined) {
		return false;
	}

	// Signal 0 is a non-destructive process-existence check. It must not enter the tree-termination path.
	if (signal === 0) {
